...
class AirInterpreter(
    ...
    override fun interpret(value: AirValue): AirValue {
        ...
        if (keyword is StringValue) {
            when (keyword.value) {
                // eval
                C.VALUE, C.VALUE_SYMBOL -> return tuple[1]
                C.QUOTE, C.QUOTE_SYMBOL -> return quote(tuple[1])
                C.EVAL, C.EVAL_SYMBOL -> return interpret(
                    interpret(tuple[1])
                )
            }
        }
        ...
        if (keyword is StringValue) {
            return interpretString(keyword.value, evalTuple)
        }
    ...
    private fun quote(v: AirValue): AirValue {
        ...
        if (keyword is StringValue) {
            when (keyword.value) {
                ...
                C.QUOTE, C.QUOTE_SYMBOL -> return quote(
                    quote(
                        t[1]
                    )
                )
                C.EVAL, C.EVAL_SYMBOL -> return interpret(t[1])
            }
        }
